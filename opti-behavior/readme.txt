...
* 📊 [Real-Time Analytics](https://optiuser.com/opti-behavior/ob-feature-dashboard/) - See what visitors are doing now: sessions, pageviews, devices, traffic sources, countries, engagement, and bot-filtered activity.
...
* ⚠️ [Errors & Performance Tracking](https://optiuser.com/opti-behavior/ob-feature-errors/) - Detect JavaScript errors, broken links, rage clicks, dead clicks, and performance issues that silently reduce trust. Pro feature.
* 🔒 [Settings & Privacy Controls](https://optiuser.com/opti-behavior/ob-feature-settings/) - Control consent, storage, reports, cleanup, languages, debugging, and privacy options from one place.
...
1. Real-time WordPress analytics dashboard with visitors, sessions, page views, average session time, scroll depth, and bounce rate KPIs.
2. Live visitor activity feed with country flags, anonymized visitor identifiers, page URLs, and an interactive real-time visitor map.
3. Traffic overview chart comparing visitors, sessions, and page views across the selected date range.
4. Top engaged users and top pages report showing session frequency, average session duration, country, page views, and interaction counts.
5. Visitor activity heatmap showing high-traffic days and hours for behavior analytics and content timing decisions.
6. Audience analytics widgets for new vs returning visitors, visited directories, and logged-in visitor authentication activity.
7. Traffic classification dashboard with human traffic, spam traffic, bot detection, crawler breakdown, and user intent segmentation.
8. Acquisition and environment analytics for referrers, countries, and browser usage.
9. Device analytics for desktop, mobile, tablet, operating systems, and screen resolutions.
10. Heatmaps overview with total heatmaps, total clicks, mobile traffic, average time on page, hottest page, click-through rate, and searchable page list.
11. Click heatmap detail view with date, country, browser, visitor type, desktop/mobile filters, page preview, and click intensity overlay.
12. Move heatmap detail view showing mouse movement trails and visitor movement patterns on a live page preview.
13. Attention heatmap detail view showing attention intensity and engagement hotspots on the selected page.
14. Scroll heatmap detail view showing scroll-depth intensity, loaded scroll points, and page-level engagement depth.
15. Post Analytics panel with entry sources, on-page visitor behavior, exit behavior, total interactions, scroll depth, bounce rate, and update time.
16. Page-level analytics with country, browser, device breakdowns, visitor trends, heatmap access, and session recording access.
17. Pro: Session Recordings list with watched status, duration, visitor activity, visited pages, referrer, country, browser, operating system, and device data.
18. Pro: Session recording page timeline showing multi-page visitor journeys with events, clicks, durations, and replay controls.
19. Pro: Session replay player with visitor metadata, event timeline, playback controls, skip-inactive mode, and page-by-page session navigation.
20. Conversion funnels dashboard showing funnel steps, completion rate, abandonment rate, device and country filters, and funnel performance cards.
21. Funnel builder modal for creating or editing URL-based funnel steps with URL match rules and step ordering.
22. Pro: User Journey Analytics with journey flow visualization, top paths, entry and exit analysis, page groups, device, browser, country, visitor, and referrer filters.
23. Pro: Errors Tracking dashboard for JavaScript errors, network errors, friction events, rage clicks, slow pages, affected sessions, performance, and broken links.
24. Pro: Form Analytics dashboard with form views, submissions, conversion rate, completion time, abandonments, activity trends, and top forms by conversion.
25. Privacy and GDPR settings comparing anonymous mode and full tracking for cookies, IP storage, geolocation, consent, session accuracy, and user journey tracking.
26. Consent banner configuration with third-party consent plugin detection, banner position controls, colors, title, and message settings.
...
= 1.7.1.2 =
Smart Insights usability release: notification cards, the Smart Insights center, and insight details now show when each signal was detected (relative time with exact date on hover). Pair with Opti-Behavior Pro 1.7.1.2 to keep both plugins on the same version.

= 1.7.1 =
Major compatibility and reliability release: unified cache/optimizer exclusion layer, early-click capture queue, tracker heartbeat warning when a cache plugin blocks tracking, A/B nonce auto-refresh, funnel/analytics fixes, and ~90% fewer admin-ajax requests. Strongly recommended for all sites.

= 1.7.0 =
Major Analytics dashboard and Heatmaps performance release: parallel/cached widgets, collapsible sections, indexed heatmap aggregates, and a User Intent classification fix. Version synced with Pro. Displayed numbers are identical and live. Recommended for all sites.

= 1.6.5 =
Completes admin-interface translations (FR, DE, ES, PT-BR, IT), adds a Funnel-to-recordings replay deep-link, and fixes the Scroll heatmap to show true scroll-reach. Recommended for all sites.

= 1.6.4 =
Fixes Dashboard KPI period-over-period trends for Bounce Rate, Session Duration, and Scroll Depth, restores the Bounce Rate sparkline, and fixes KPI sparkline tooltip overlap. Recommended for all sites.

= 1.6.3 =
Improves mobile admin display compatibility, Dashboard and Heatmap metric accuracy, and data-safety/spam-filtered reporting consistency. Recommended for all sites.

= 1.6.2 =
Adds Smart Insights to Scheduled Reports and keeps report metrics aligned with Traffic Behavior spam-filter settings. Recommended for all sites using scheduled email reports.

= 1.6.1 =
Fixes a Dashboard Traffic Classification mismatch where Human Traffic could lag behind the Sessions KPI when spam filtering and dashboard caches were active.

= 1.6.0 =
Major analytics-consistency and cleanup release. Adds canonical spam-filtered reporting plus upgraded Smart Data Cleanup controls. Recommended for all sites before reviewing Dashboard, Heatmap, Funnel, A/B, and Smart Insights reports.

= 1.3.2 =
Adds a localized deactivation feedback modal on plugins.php and fail-open survey relay to OptiUser API while preserving normal deactivation flow. Recommended for all Free users.

= 1.3.1 =
Fixes Free click heatmap detail loading when Pro is disabled. Recommended for all Free users and Free+Pro sites that may deactivate Pro.

= 1.2.9 =
Adds per-page heatmap data reset for redesigned pages, a branded confirmation modal, optimized Frontend Stats Bar polish, advanced GDPR banner controls, and filtering/cache fixes. Recommended for all users.

= 1.2.8.1 =
Fixes A/B Visual Editor opening and preview flows when browser popup blockers or privacy extensions are active. Recommended for all A/B testing users.

= 1.2.8 =
Adds expanded A/B testing tools, improved experiment reporting, and more reliable page-split/cross-page goal attribution. Recommended for all A/B testing users.

= 1.2.4 =
Fixes all WordPress Plugin Check issues, GDPR consent banner visibility, and adds smart auto-cleanup defaults on fresh installs.

= 1.2.3 =
Major release with Frontend Stats Bar, SMTP email config, full cache plugin compatibility (LiteSpeed, WP Rocket, Autoptimize, SG Optimizer), 10 Smart Data Cleanup bug fixes, live visitors query optimization, and cron monitor. Recommended for all users.

= 1.2.1 =
Critical fix for heatmap iframe not loading on Hostinger and other CDN-hosted sites. Server-side proxy bypasses X-Frame-Options restrictions. Also fixes slider height issues and fatal error with older Pro versions. Recommended for all users.

= 1.2.0 =
New Pro trial banner with license-aware visibility, ipwho.is geolocation fallback for high-traffic sites, and fixes for "Unknown" country in Real-time Visitors. Recommended for all users.

= 1.1.0 =
Major stability release with anonymous install tracker, autoloader conflict fixes for Free/Pro coexistence, heatmap sessions accuracy improvements, SQL fixes, and full WordPress Plugin Check compliance (0 errors, 0 warnings). Recommended for all users.

= 1.0.9 =
Major release with Smart Data Cleanup, comprehensive tooltips, Form Analytics foundation, 4 new languages (German, Italian, Spanish, Portuguese), Lucide SVG icons throughout, 15+ bug fixes for heatmaps/funnels/stats, and 100% WordPress Plugin Check compliance.
...
= 1.0.5 =
WordPress.org submission update: Fixes debug code, timezone handling, and i18n compliance. All users should update for WordPress coding standards compliance.

= 1.0.4 =
Major update with file storage system, bot detection, and performance optimizations. Recommended for all users, especially high-traffic sites.

= 1.0.3 =
Important security update with enhanced SQL security. All users should update.

= 1.0.0 =
Initial release. Install to start tracking user behavior with powerful heatmaps and analytics.
...
Developed by [OptiUser](https://optiuser.com/)
